...
import sys
from anomaly_stat import AnomalyStat
...
def compute_dist_events(m_train_set):
    ...
    for event_idx in range(1,8):
        ...
        m_std[event_idx]  = np.std(counter) + 10e-6
...
with tf.Graph().as_default():
    ...
    # cell = tf.nn.rnn_cell.MultiRNNCell([tf.nn.rnn_cell.GRUCell(memory_dim) for _ in range(3)])
    ...
    for n in range(start_idx, start_idx+bulk, 1):
        ...
        with tf.Session(config=config) as sess:
            ...
            for x in range(len(test_set)):  # for each day 
                ...
                flag_answer, gt_type = AnomalyStat.is_anomaly(anomaly_stat, date[x], n)
                if ploss >= 0.9:
        ...
        print("user {} : TP{}, TN{}, FP{}, OT{} : Acc({}) Recall({}) DONE".format(n, user_true_positive, user_true_negative, user_false_positive, user_other,
                                                  user_true_positive / (user_true_positive+user_false_positive+1.), 
                                                  user_true_positive / (user_true_positive+user_true_negative+1.)))
